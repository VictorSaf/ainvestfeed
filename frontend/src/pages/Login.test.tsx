...
describe('Login', () => {
  ...
  it('stores access token on success', async () => {
    ...
    vi.stubGlobal('fetch', vi.fn().mockResolvedValue({ ok: true, json: () => Promise.resolve({ data: { tokens } }) } as any));
  ...
  it('shows error on failure', async () => {
    vi.stubGlobal('fetch', vi.fn().mockResolvedValue({ ok: false, json: () => Promise.resolve({ error: { message: 'Bad creds' } }) } as any));
    render(<Login />);
    fireEvent.change(screen.getByTestId('email'), { target: { value: 'x@example.com' } });
